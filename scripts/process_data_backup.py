#!/usr/bin/env python3
"""
Türkiye Sağlık Kuruluşları - Ver    # İl adını normalize et    # İl ad    }
    ...
    # Türkçe karakterleri ASCII'ye çevir
    il_ascii = il_normalized.replace('İ', 'I').replace('Ş', 'S').replace('Ğ', 'G')
    il_ascii = il_ascii.replace('Ü', 'U').replace('Ö', 'O').replace('Ç', 'C')l_normalized = il_adi.upper().strip()
    
    # Unicode combining characters temizle (örn: ̇ karakter kodu 775)
    import unicodedata
    il_normalized = unicodedata.normalize('NFD', il_normalized)
    il_normalized = ''.join(c for c in il_normalized if unicodedata.category(c) != 'Mn')
    il_normalized = unicodedata.normalize('NFC', il_normalized)
    ...
    alternatifler = {
        ...
        'URFA': 'ŞANLIURFA',
        'SANLIURFA': 'ŞANLIURFA',
        'HAKKARI': 'HAKKARİ',  # Combining character temizlenmiş HAKKARİ
    }
    
    if il_normalized in alternatifler:
        il_normalized = alternatifler[il_normalized]
    
    return il_kodlari.get(il_normalized, 0)aw klasöründeki verileri temizler, standardize eder ve tek dosyada birleştirir.
"""
